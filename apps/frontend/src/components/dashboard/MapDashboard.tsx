...
import type { MapInfo, Session } from "@stream-manager/shared";
...
export default function MapDashboard({
  ...
  const [maps, setMaps] = useState<MapInfo[]>(
    (session?.mapInfo as MapInfo[]) || []
  );
  const [hasChanges, setHasChanges] = useState(false);
  const [isBestOf, setIsBestOf] = useState(session?.bestOf || false);
  const [previousMaps, setPreviousMaps] = useState<MapInfo[]>(
    (session?.mapInfo as MapInfo[]) || []
  );
  ...
  const updateMap = (id: number, field: keyof MapInfo, value: string | null) => {
    ...
    const newMaps = maps.map((map) => {
      if (map.id === id) {
        ...
        if (
          field === "winner" &&
          (value === "team1" || value === "team2" || value === null)
        ) {
          if (value === null) {
            return {
              ...map,
  ...
  const handleSave = useCallback(() => {
    ...
    setHasChanges(false);
  }, [maps, previousMaps, isBestOf, session?.game, session?.team1Ban, session?.team2Ban, mutateFn]);
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0 md:space-x-4">
            <div className="flex items-center space-x-2">
              ...
              <Select
                ...
                onValueChange={(value) => updateNumberOfMaps(parseInt(value))}
              >
                <SelectTrigger
                  id="numberOfMaps"
                  className="w-[120px] bg-gray-600 border-gray-500"
                >
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      ...
                      updateMap(
                        map.id,
                        "winner",
                        updatedValue as "team1" | "team2" | null
                      );
                    }}
                  >
                    <SelectTrigger
