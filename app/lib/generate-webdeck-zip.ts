...
import { OverwatchCharacters } from "./characters";
import { OverwatchMaps } from "./maps";
 ...
 */
function createCharacterBanButton(character: typeof OverwatchCharacters[0], team: "Left" | "Right"): any {
 ...
 */
function createMapButton(map: typeof OverwatchMaps[0]): any {
...
function generateConfigJson(originalConfig: any): any {
    ...
    const generateRoleButtons = (role: "Tank" | "Damage" | "Support", team: "Left" | "Right") => {
        const characters = OverwatchCharacters.filter(c => c.role === role);
        const buttons = characters.map(char => createCharacterBanButton(char, team));
...
export async function generateWebDeckZip(sessionId: string): Promise<Buffer> {
    ...
    try {
        const characterImageFiles = await readDirectoryRecursive(characterImagesDir, characterImagesDir);
    ...
    try {
        const mapImageFiles = await readDirectoryRecursive(mapImagesDir, mapImagesDir);
