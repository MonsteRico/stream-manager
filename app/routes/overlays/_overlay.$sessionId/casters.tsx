...
function CastersOverlay() {
    ...
    useEffect(() => {
        if (placeholderOneRef.current && placeholderTwoRef.current) {
            const { top: topOne, left: leftOne, width: widthOne, height: heightOne } = placeholderOneRef.current.getBoundingClientRect();
            ...
            const { top: topTwo, left: leftTwo, width: widthTwo, height: heightTwo } = placeholderTwoRef.current.getBoundingClientRect();
