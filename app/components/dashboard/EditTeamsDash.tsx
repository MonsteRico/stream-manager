	...
	return (
		<Card className="bg-gradient-to-r from-gray-800 to-gray-900 text-white">
			...
			<CardContent>
						...
						<div key={team} className="space-y-4">
							<div className="flex items-center justify-between">
								{editingTeam === team ? (
									...
									<h2 className="text-2xl font-bold cursor-pointer hover:text-gray-300 transition-colors duration-200 flex-grow mr-4" onClick={() => setEditingTeam(team as 1 | 2)}>
										{formState[`team${team}DisplayName` as "team1DisplayName" | "team2DisplayName"]} <Edit2 className="inline-block w-4 h-4 ml-2" />
									</h2>
								)}
								<div className="flex items-center">
									<div className="flex flex-col mr-2">
										<Button size="sm" variant="outline" onClick={() => handleScoreChange(team as 1 | 2, true)} className="px-2 py-0 h-6">
											<ChevronUp className="h-4 w-4" />
										</Button>
										<Button size="sm" variant="outline" onClick={() => handleScoreChange(team as 1 | 2, false)} className="px-2 py-0 h-6 mt-1">
											<ChevronDown className="h-4 w-4" />
										</Button>
									</div>
									<span className="text-3xl font-bold">{formState[`team${team}Score` as "team1Score" | "team2Score"]}</span>
								</div>
							</div>
				...
				<Button onClick={handleSave} className="mt-6 w-full bg-blue-600 hover:bg-blue-700" disabled={!hasChanges}>
					Save Changes
				</Button>
			</CardContent>
		</Card>
...
function LogoEdit({ team, formState, setFormState }: { team: 1 | 2; formState: NewSession; setFormState: React.Dispatch<React.SetStateAction<NewSession>> }) {
				...
				<PopoverTrigger asChild>
					<Button variant="outline" className="w-full h-10">
						{formState[`team${team}Logo` as "team1Logo" | "team2Logo"] ? (
							<img src={formState[`team${team}Logo` as "team1Logo" | "team2Logo"] ?? ""} alt={`Team ${team} logo`} className="w-full h-full object-contain" />
						) : (
							"Set Logo"
						)}
					</Button>
